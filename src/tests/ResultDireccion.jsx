...
import results from '../data/results_direccion';
import styles from '../styles/testStyles/resultDireccion.module.css';

const STORAGE_RESULT_KEY = 'tdr_dir_result';
...
const ResultDireccion = () => {
  ...
  useEffect(() => {
    ...
    if (k && results[k]) {
      ...
      navigate('/test-direccion/quiz', { replace: true });
    }
    ...
    <section className={styles.shell}>
      ...
      <div className={styles.layout}>
        ...
        <main className={styles.main}>
          {data.diagnostico && (
            <p className={styles.copy}>{data.diagnostico}</p>
          )}
          ...
          <aside className={styles.aside}>
            <div className={styles.ctaCard}>
              ...
              {data.cta.descripcion && (
                <p className={styles.copy}>{data.cta.descripcion}</p>
              )}

              {data.cta.frase && (
                <blockquote className={styles.quote}>{data.cta.frase}</blockquote>
              )}
